...
1. **Свветодиод, который можно переключить** 
2. **Пользовательский ввод (кнопка)** 
3. **Режимы работы: моргание, бегущий огонек, шахматка** 
...
В лабораторной работе №5 успешно разработана объектно-ориентированная система управления светодиодами для STM32. Реализована модульная архитектура с применением принципов SOLID, обеспечивающая чёткое разделение ответственности между компонентами.

Созданы абстрактные интерфейсы для кнопки (IButton) и светодиодов (ILedToggable, ILedOnOff), что позволило отделить логику управления от аппаратной реализации. Реализованы три режима работы: "Бегущий огонь", "Синхронное мигание" и "Шахматный порядок" через иерархию классов с базовым CommonMode.

Архитектура системы демонстрирует высокую расширяемость — добавление новых режимов требует только создания класса, наследующего IMode. Использование шаблонов (OptimizedLed) обеспечило compile-time оптимизацию, а полиморфизм позволил создавать гибкие и поддерживаемые решения.